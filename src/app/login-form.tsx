...
function EyeIcon({ hidden }: { hidden: boolean }) {
  return hidden ? (
    <svg aria-hidden="true" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="m3 3 18 18" /><path d="M10.6 10.7a2 2 0 0 0 2.7 2.7" /><path d="M9.9 4.2A10.7 10.7 0 0 1 12 4c5.5 0 9 5.2 9 5.2a15.5 15.5 0 0 1-2.1 2.7" /><path d="M6.6 6.6A15.6 15.6 0 0 0 3 9.2S6.5 14.4 12 14.4c.8 0 1.6-.1 2.3-.3" />
    </svg>
  ) : (
    <svg aria-hidden="true" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      <path d="M3 12s3.5-5.2 9-5.2 9 5.2 9 5.2-3.5 5.2-9 5.2S3 12 3 12Z" /><circle cx="12" cy="12" r="2.4" />
    </svg>
  );
}
...
export function LoginForm() {
  ...
  const supabase = useMemo(() => hasSupabaseBrowserEnv() ? createClient() : null, []);
      ...
      <section className="login-card" aria-labelledby="login-title">
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="field-group">
            ...
            <input id="email" name="email" type="email" inputMode="email" autoComplete="username" placeholder="nama@institusi.ac.id" maxLength={254} value={email} aria-invalid={Boolean(emailError)} aria-describedby="login-message" onBlur={() => setEmailTouched(true)} disabled={isLoading} onChange={(event) => { setEmail(event.target.value); setNotice(""); setNoticeType("neutral"); }} />
          </div>
          <div className="field-group">
          ...
          <button className="forgot-password" type="button" disabled={isLoading} onClick={handleForgotPassword}>Lupa kata sandi?</button>
          <p id="login-message" className={`login-message ${messageType}`} role={messageType === "error" ? "alert" : "status"} aria-live="polite">{message}</p>
          <button className="submit-button" type="submit" disabled={isLoading}>{isLoading ? "Memeriksa..." : "Masuk"}</button>
        </form>
      </section>
    </main>
  );
