...
type AddMode = "picker" | "bulk" | "siap" | null;
...
function parseImportCsv(text: string, existingEmails: Set<string>, fallbackRole?: AssignableRole): ImportCandidate[] {
  ...
  if (nameIndex < 0 || emailIndex < 0 || (roleIndex < 0 && !fallbackRole)) {
    return [{
      line: 1,
      displayName: "",
      email: "",
  ...
  return lines.slice(1, MAX_IMPORT + 1).map((line, offset) => {
    ...
    const roles = parseRoles(roleIndex >= 0 ? cells[roleIndex] ?? "" : "", fallbackRole);
    let error: string | undefined;

    if (!displayName) error = "Nama kosong";
    else if (!/^\S+@\S+\.\S+$/.test(email)) error = "Email tidak valid";
...
export function UserAccessControls({ initialUsers }: UserAccessControlsProps) {
  ...
  const [siapRole, setSiapRole] = useState<AssignableRole>("mahasiswa");
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape" && !importing) setMode(null);
    };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [importing, mode]);

  function resetImport(nextMode: AddMode) {
    setCandidates([]);
    setFileName("");
    setReport("");
    setFailures([]);
  ...
  function openManualEditor() {
    const originalButton = actionHost
      ? [...actionHost.querySelectorAll<HTMLButtonElement>("button")]
          .find((button) => !button.classList.contains("obe-user-add-button") && button.textContent?.includes("Tambah pengguna"))
      : null;
    setMode(null);
    originalButton?.click();
  }

  async function readCsv(file: File, fallbackRole?: AssignableRole) {
    setFileName(file.name);
    setReport("");
    setFailures([]);
    const text = await file.text();
    setCandidates(parseImportCsv(text, existingEmails, fallbackRole));
  }
  ...
  const trigger = actionHost ? createPortal(
    <button className="obe-user-add-button" onClick={() => resetImport("picker")} type="button">
  ...
  return (
    <>
      ...
      {info}
      {mode ? createPortal(
        <div className="obe-user-modal-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget && !importing) setMode(null); }}>
          <section aria-labelledby="obe-user-modal-title" aria-modal="true" className="obe-user-modal" role="dialog">
            <div className="obe-user-modal-head">
              <div>
                ...
                <h2 id="obe-user-modal-title">{mode === "picker" ? "Tambah pengguna" : mode === "bulk" ? "Impor massal" : "Sinkron dari SIAP"}</h2>
              </div>
              <button aria-label="Tutup" disabled={importing} onClick={() => setMode(null)} type="button">×</button>
            </div>

            {mode === "picker" ? (
              <div className="obe-user-methods">
                <button onClick={openManualEditor} type="button"><strong>Buat satu akun</strong><span>Nama, email UNDIP, dan peran.</span><b>→</b></button>
                <button onClick={() => resetImport("bulk")} type="button"><strong>Impor massal</strong><span>CSV sampai {MAX_IMPORT} akun per batch.</span><b>→</b></button>
                <button onClick={() => resetImport("siap")} type="button"><strong>Sinkron dari SIAP</strong><span>Login SIAP dulu, lalu impor hasil ekspor.</span><b>→</b></button>
              </div>
            ) : null}

            {mode === "bulk" ? (
              <div className="obe-import-body">
                <div className="obe-import-note"><strong>Format sederhana</strong><span>CSV: <code>nama,email,peran</code>. Peran: mahasiswa, dosen, GPM, atau Kaprodi. Multi-role gunakan tanda |.</span></div>
                <label className="obe-file-box">
                  <strong>{fileName || "Pilih file CSV"}</strong>
                  <span>Setiap akun yang berhasil dibuat akan menerima alur aktivasi email OBELIKS.</span>
            ...
            {mode === "siap" ? (
              <div className="obe-import-body">
                ...
                <label className="obe-check"><input checked={siapLoggedIn} onChange={(event) => setSiapLoggedIn(event.target.checked)} type="checkbox" /><span>Saya sudah login ke SIAP UNDIP</span></label>
                <div className="obe-siap-step">
                  <span>2</span><div><strong>Ekspor data pengguna dari SIAP</strong><small>Untuk sementara sinkronisasi memakai file ekspor; tidak ada scraping akun.</small></div>
                </div>
                <label className="obe-role-default"><span>Jenis data</span><select disabled={importing} onChange={(event) => setSiapRole(event.target.value as AssignableRole)} value={siapRole}><option value="mahasiswa">Mahasiswa</option><option value="dosen">Dosen</option><option value="gpm">GPM</option><option value="kaprodi">Kaprodi</option></select></label>
                <label className={`obe-file-box ${!siapLoggedIn ? "disabled" : ""}`}>
                  <strong>{fileName || "Unggah CSV hasil ekspor"}</strong>
                  <span>Header minimal: nama,email. NIP/NIM boleh ada untuk referensi preview, tetapi belum disimpan sebagai field akun pada MVP ini.</span>
                  <input accept=".csv,text/csv" disabled={!siapLoggedIn || importing} onChange={(event) => { const file = event.target.files?.[0]; if (file) void readCsv(file, siapRole); }} type="file" />
                </label>
                <div className="obe-student-note"><strong>Mahasiswa</strong><span>Akun mahasiswa disiapkan di sini. KRS/peserta kelas tetap menjadi data kelas agar workflow RPS → asesmen → CPMK/CPL tidak tercampur dengan administrasi akun.</span></div>
              </div>
            ) : null}

            {mode !== "picker" && candidates.length ? (
            ...
            {report ? <div className="obe-import-report"><strong>{report}</strong>{failures.map((failure) => <small key={failure}>{failure}</small>)}<button onClick={() => window.location.reload()} type="button">Muat ulang daftar</button></div> : null}

            {mode !== "picker" ? (
              <div className="obe-user-modal-actions">
                <button disabled={importing} onClick={() => setMode("picker")} type="button">Kembali</button>
                <button className="primary" disabled={!stats.ready || importing} onClick={() => void importUsers()} type="button">{importing ? "Memproses…" : `Buat ${stats.ready} akun`}</button>
              </div>
            ) : null}
          </section>
        </div>,
      ...
      <style jsx global>{`
        ...
        .obe-user-modal { width: min(650px, 100%); max-height: min(760px, calc(100vh - 36px)); overflow: auto; border-radius: 16px; background: #fff; box-shadow: 0 24px 70px rgba(8,25,38,.28); color: #17212b; }
        ...
        .obe-user-methods > button { display: grid; grid-template-columns: minmax(0,1fr) auto; gap: 3px 12px; border: 1px solid #dfe6eb; border-radius: 12px; background: white; padding: 14px 15px; text-align: left; cursor: pointer; }
        ...
        .obe-user-methods b { grid-column: 2; grid-row: 1 / span 2; align-self: center; color: #08766f; }
        .obe-import-body { display: grid; gap: 12px; padding: 17px 20px 8px; }
        .obe-import-note, .obe-student-note { display: grid; gap: 4px; border-radius: 10px; background: #f5f8fa; padding: 11px 12px; }
        .obe-import-note strong, .obe-student-note strong { font-size: 10px; }
        .obe-import-note span, .obe-student-note span { color: #667785; font-size: 9px; line-height: 1.45; }
        ...
        .obe-role-default { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-left: 37px; }
        .obe-role-default span { color: #596975; font-size: 9px; font-weight: 700; }
        .obe-role-default select { border: 1px solid #d7e0e5; border-radius: 8px; background: white; padding: 7px 9px; font-size: 10px; }
        ...
        @media (max-width: 640px) {
          ...
          .obe-user-modal { width: 100%; max-height: 90vh; border-radius: 16px 16px 0 0; }
