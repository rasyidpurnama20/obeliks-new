...
test("CSV import is capped at 100 and validates before mutation", () => {
  ...
  assert.match(controls, /mode === "siap"[\s\S]*createCustomManagedUser/);
...
test("SIAP onboarding is server-enforced as Mahasiswa-only and uses a bootstrap password", () => {
  ...
  assert.match(customActions, /must_change_password: true/);
...
test("bootstrap accounts cannot reach dashboard before replacing the initial password", () => {
  ...
  assert.match(login, /user_metadata\?\.must_change_password === true/);
  assert.match(login, /router\.replace\("\/reset-password\?required=1"\)/);
  assert.match(home, /must_change_password === true[\s\S]*redirect\("\/reset-password\?required=1"\)/);
  assert.match(dashboardEntry, /must_change_password === true[\s\S]*redirect\("\/reset-password\?required=1"\)/);
  assert.match(reset, /data: \{ must_change_password: false \}/);
...
test("custom account action validates Superadmin and never logs the bootstrap password value", () => {
  ...
  const metadataBlock = customActions.slice(customActions.indexOf("p_metadata:"), customActions.indexOf("});", customActions.indexOf("p_metadata:")));
